...
//		Serial.print(press>>2);       // Convert into Pa       
	...
	if(GPS.new_data==1) {
		...
		#endif
		GPS.new_data=0;
		Serial.write(0x55);  // This is the message preamble
		Serial.write(0xAA);  // This is the message preamble
                Serial.write(0x01);

		templong = GPS.longitude; //Longitude *10**7 in 4 
                b = (byte *) &templong;
                Serial.write(b,4);
		GPS_chk ^= templong&0xff;
		GPS_chk ^=(templong>>8)&0xff;
		GPS_chk ^=(templong>>16)&0xff;
		GPS_chk ^=(templong>>24)&0xff;
		...
		tempint=GPS.altitude;   // Altitude MSL in meters * 10 in 2 bytes
                b = (byte *) &tempint;
                Serial.write(b,2);
		GPS_chk ^=tempint&0xff;
		GPS_chk ^=(tempint>>8)&0xff;
		...
		GPS_chk ^=(tempint>>8)&0xff;
                ...
                b = (byte *) &GPS.time;
                Serial.write(b,4);
		GPS_chk ^=GPS.time&0xff;
		GPS_chk ^=(GPS.time>>8)&0xff;
		GPS_chk ^=(GPS.time>>16)&0xff;
		GPS_chk ^=(GPS.time>>24)&0xff;

                Serial.write(GPS.num_sats);
                GPS_chk ^=GPS.num_sats&0xff;
                
                Serial.write(GPS.status());
                GPS_chk ^=GPS.status()&0xff;
                
      	        Serial.write(GPS_chk);
  
	}
      
  	// This section outputs the IMU message
  	Serial.write(0x55);  // This is the message preamble
  	Serial.write(0xAA);  // This is the message preamble
  	Serial.write(0x02);  // Message Type    

        b = (byte *) &Gyro_Vector[0];
        Serial.write(b,4);
        IMU_chk ^= b[0]; 
  	IMU_chk ^= b[1];
  	IMU_chk ^= b[2];
  	...
  	IMU_chk ^= b[3];
        ...
        //Serial.write(Accel_Vector[0]);
        b = (byte *) &Accel_Vector[0];
        Serial.write(b,4);
        IMU_chk ^= b[0]; 
  	IMU_chk ^= b[1];
  	IMU_chk ^= b[2];
  	IMU_chk ^= b[3];
        ...
        //Serial.write(Accel_Vector[1]);  
        b = (byte *) &Accel_Vector[1];
        Serial.write(b,4);
        IMU_chk ^= b[0]; 
  	IMU_chk ^= b[1];
  	IMU_chk ^= b[2];
  	IMU_chk ^= b[3];
        ...
        //Serial.write(Accel_Vector[2]);  
        b = (byte *) &Accel_Vector[2];
        Serial.write(b,4);
        IMU_chk ^= b[0]; 
  	IMU_chk ^= b[1];
  	IMU_chk ^= b[2];
  ...
  /*
  	...
  	for (int i=0;i<ck+2;i++) Serial.write (IMU_buffer[i]);
